...
const FetchingDataPricing = async (req, res) => {
    try {
        Pricing = await pricingPlanModels.findAll({})
...
const FetchPricingDataById = async (req, res) => {
    try {
        ...
        res.status(200).json("Id and data Found : ", FetchPricingId)
    } catch (error) {
        console.log(error.message);
        res.status(500).send(`Fetch data ${FetchPricingId} tidak berhasil`);
    }
}
